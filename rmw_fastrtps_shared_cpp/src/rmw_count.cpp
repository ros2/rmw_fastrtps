  ...
  size_t * count)
{
  ...
  ret = 
    common_context->graph_cache.get_writer_count(mangled_rq_service_name, &number_of_request_subscribers);
  if (ret != RMW_RET_OK) {
    return ret;
  }
  size_t number_of_response_publishers = 0; 
  ret =
    common_context->graph_cache.get_reader_count(mangled_rp_service_name, &number_of_response_publishers);
  if (ret != RMW_RET_OK) {
    return ret;
  }
