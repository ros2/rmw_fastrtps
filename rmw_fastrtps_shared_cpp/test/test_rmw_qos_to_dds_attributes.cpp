...
static const rosidl_type_hash_t ZeroTypeHash = rosidl_get_zero_initialized_type_hash();
...
TEST_F(GetDataReaderQoSTest, test_unknown_history_policy_conversion_fails) {
  ...
  EXPECT_FALSE(get_datareader_qos(qos_profile_, ZeroTypeHash, subscriber_qos_));
...
TEST_F(GetDataReaderQoSTest, nominal_conversion) {
  ...
  EXPECT_TRUE(get_datareader_qos(qos_profile_, ZeroTypeHash, subscriber_qos_));
...
TEST_F(GetDataWriterQoSTest, nominal_conversion) {
  ...
  EXPECT_TRUE(get_datawriter_qos(qos_profile_, ZeroTypeHash, publisher_qos_));
