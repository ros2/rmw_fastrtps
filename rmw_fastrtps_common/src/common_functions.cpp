...
#include "rmw_fastrtps_common/TypeSupport_impl.h"
...
rmw_ret_t common_rmw_init()
{
  return RMW_RET_OK;
}

rmw_node_t * common_rmw_create_node(const char * name, size_t domain_id)
{
  if (!name) {
...
rmw_ret_t common_rmw_destroy_node(rmw_node_t * node)
{
  if (!node) {
...
rmw_publisher_t * common_rmw_create_publisher(const rmw_node_t * node,
...
rmw_ret_t common_rmw_destroy_publisher(rmw_node_t * node, rmw_publisher_t * publisher)
{
  if (!node) {
    RMW_SET_ERROR_MSG("node handle is null");
    return RMW_RET_ERROR;
...
rmw_ret_t common_rmw_publish(const rmw_publisher_t * publisher, const void * ros_message)
{
  assert(publisher);
  assert(ros_message);
...
rmw_subscription_t * common_rmw_create_subscription(const rmw_node_t * node,
  const rosidl_message_type_support_t * type_supports,
...
rmw_ret_t common_rmw_destroy_subscription(rmw_node_t * node, rmw_subscription_t * subscription)
{
  if (!node) {
    RMW_SET_ERROR_MSG("node handle is null");
    return RMW_RET_ERROR;
...
rmw_ret_t common_rmw_take(const rmw_subscription_t * subscription, void * ros_message, bool * taken)
{
  assert(subscription);
  assert(ros_message);
  assert(taken);
...
rmw_ret_t common_rmw_take_with_info(
...
rmw_guard_condition_t * common_rmw_create_guard_condition()
{
...
rmw_ret_t common_rmw_destroy_guard_condition(rmw_guard_condition_t * guard_condition)
{
  if (guard_condition) {
...
rmw_ret_t common_rmw_trigger_guard_condition(const rmw_guard_condition_t * guard_condition_handle)
{
...
rmw_client_t * common_rmw_create_client(const rmw_node_t * node,
...
rmw_ret_t common_rmw_send_request(const rmw_client_t * client,
...
rmw_ret_t common_rmw_take_request(const rmw_service_t * service,
...
rmw_ret_t common_rmw_take_response(const rmw_client_t * client,
  rmw_request_id_t * request_header,
  void * ros_response,
  bool * taken)
{
...
rmw_ret_t common_rmw_send_response(const rmw_service_t * service,
  rmw_request_id_t * request_header,
  void * ros_response)
{
  assert(service);
...
rmw_service_t * common_rmw_create_service(const rmw_node_t * node,
  const rosidl_service_type_support_t * type_supports,
  const char * service_name, const rmw_qos_profile_t * qos_policies)
{
...
rmw_ret_t common_rmw_destroy_service(rmw_node_t * node, rmw_service_t * service)
{
  (void)node;
  if (!service) {
...
rmw_ret_t common_rmw_destroy_client(rmw_node_t * node, rmw_client_t * client)
{
  (void)node;
  if (!client) {
...
rmw_ret_t common_rmw_wait(rmw_subscriptions_t * subscriptions,
